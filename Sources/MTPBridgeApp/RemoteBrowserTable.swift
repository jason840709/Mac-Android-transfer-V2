...
struct RemoteBrowserTable: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        tableView.rowHeight = textSize.tableRowHeight
        tableView.intercellSpacing = NSSize(width: 8, height: 1)
        tableView.columnAutoresizingStyle = .lastColumnOnlyAutoresizingStyle
        tableView.autosaveName = "AndroidTransferV2.RemoteBrowserTable.v3"
        ...
        for specification in ColumnSpecification.all {
            ...
            column.headerCell.font = NSFont.systemFont(ofSize: textSize.tableHeaderPointSize, weight: .medium)
        ...
        context.coordinator.applyTypography(to: tableView)
    ...
    final class Coordinator: NSObject, NSTableViewDataSource, NSTableViewDelegate {
        ...
        private var appliedTextSize: InterfaceTextSize?
        ...
        func applyTypography(to tableView: NSTableView) {
            guard appliedTextSize != parent.textSize else { return }
            appliedTextSize = parent.textSize
            tableView.rowHeight = parent.textSize.tableRowHeight
            for column in tableView.tableColumns {
                column.headerCell.font = NSFont.systemFont(
                    ofSize: parent.textSize.tableHeaderPointSize,
                    weight: .medium
                )
            }
            if !objects.isEmpty {
        ...
        private func nameCell(in tableView: NSTableView, object: MTPObject) -> NSTableCellView {
            ...
            cell.textField?.font = NSFont.systemFont(ofSize: parent.textSize.tablePointSize)
            let image = NSImage(
        ...
        private func textCell(
            ...
            cell.textField?.font = monospacedDigits
                ? NSFont.monospacedDigitSystemFont(ofSize: parent.textSize.tablePointSize, weight: .regular)
                : NSFont.systemFont(ofSize: parent.textSize.tablePointSize)
        ...
        private func iconColor(for category: MTPObjectCategory) -> NSColor {
            switch category {
            case .folder: return AppPalette.folderNSColor
            case .image: return AppPalette.imageNSColor
            case .video: return AppPalette.videoNSColor
            case .audio: return AppPalette.audioNSColor
            case .pdf: return AppPalette.pdfNSColor
            case .archive: return AppPalette.archiveNSColor
            case .document: return AppPalette.documentNSColor
            case .application: return AppPalette.applicationNSColor
